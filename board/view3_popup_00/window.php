...
if($popup_list['view3_check_02'] == '1') { // 팝업 컨텐츠 1.이미지 2.동영상
	...
	$show_img_array = explode("||",$popup_list['view3_file']);
	$img_size = getimagesize('../../upload/'.$popup_table.$show_img_array['0']);
	$layer_x = $img_size[0];
	$layer_y = $img_size[1];
	$popup_content .= '<img src="'.$pc.'/upload/'.$popup_table.$show_img_array['0'].'" alt="'.$popup_list['view3_command_01'].'" width="'.$layer_x.'" height="'.$layer_y.'"';

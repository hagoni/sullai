	...
	Geomap.prototype.setPlaceMarkers = function(requestUrl, data, callback) {
		...
		$.post(requestUrl, data, function(response) {
			...
			for(var i=0; i<length; i++) {
				...
				if(typeof placeInfo.geocode === 'object') _this.marking(placeInfo.geocode , placeInfo, length);
				else {
			...
			if(typeof data['search'] === 'string' && typeof data['select_list'] === 'string') {
				if(data['select_list'].indexOf('title_01') === -1 && length !== 1) {
 ...
 ****************************************/
	...
	var Geomapping = function() {
		...
		var	container = 'placeLoadMap',
			markerImg = CONST_ROOT + '/design/other/marker.png',
		...
		this.initialize = function() {
			...
			gmap.initialize(container, options, function() {
				if(placeSearch === '') gmap.setPlaceMarkers(ajaxFilePath + '/place_find.php', {board: CONST_BOARD});
				else gmap.setPlaceMarkers(ajaxFilePath + '/place_find.php', {board: CONST_BOARD, search: placeSearch, select_list: fieldList});
			});
		};
	...
	var GeomapFind = function() {
		...
		this.local1Action = function(e) {
			...
			if(typeof depth2 === 'object') {
				...
				var query = {board: CONST_BOARD, sca: 'all', search: addr, select_list: 'addr_road||addr_number||addr_detail'};
			} else {
				var query = {board: CONST_BOARD};
			}
		...
		this.findAction = function(e) {
			...
			if(value !== '') {
				var query = {board: CONST_BOARD, sca: 'all', search: value, select_list: 'title_01||addr_road||addr_number||addr_detail'};
				gmap.setPlaceMarkers(requestUrl, query);
				_this.updateList(query);
			}
			e.preventDefault();

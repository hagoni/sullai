<? include_once "list_high.php"?>
		<div class="view_conts">
            ...
            <?
            $sql = "select * from ".TABLE_LEFT."event_01 where view3_use = 1 and view3_idx = {$_REQUEST['idx']}";
            ...
            <div id="boardWrap">
                ...
                <!-- <ul class="tabmenu fs_def">
                    <li<?if($view3_tab == '' || $view3_tab == '1'){echo ' class="on"';}?>><a href="<?=URL_PATH.'?'.get("page||tab||idx","tab=1&idx=".$_REQUEST['store']);?>">진행 중 이벤트</a></li>
					<li<?if($view3_tab == '2'){echo ' class="on"';}?>><a href="<?=URL_PATH.'?'.get("page||tab||idx","tab=2&idx=".$_REQUEST['store']);?>">완료된 이벤트</a></li>
					<li<?if($view3_tab == '3'){echo ' class="on"';}?>><a href="<?=URL_PATH.'?'.get("page||tab||idx","tab=3&idx=".$_REQUEST['store']);?>">전체 이벤트</a></li>
            	</ul> -->
				...
				?>
				<img src="<?=$root?>/upload/event_01<?=$list_img[$i]?>" alt="">
				<?
				}
	            ...
	            <?
	            ######################################################################################################################################################
	            include_once(BOARD_INC.'/setup_map_bottom.php');
	            ######################################################################################################################################################
	            ?>
				</div>
            </div>

...
(function($) {
	...
	window.mobilePopup = function(type, options) {
		...
		this.m_type01 = function() {
			function close() {
				$('#popup' + type + 'Block').remove();
                container.remove();
                swiper.destroy();
				if(typeof opt.callback === 'function' && $('[id^=popup]').length === 0) opt.callback();
            }
			var container = $('#popup' + type), swiper;
			...
			body.append('<div id="popup' + type + 'Block"></div>');
			$('#popupX' + type).click(close);
		};

		this.m_type02 = function() {
			function close(e) {
                if($(this).is('#popupCheck' + type) === true) _this.setCookie('popup_' + type, 'hide', 1);
                container.animate({opacity: 0, top: '-100%'}, 500, function() {
                    container.remove();
					swiper.destroy();
					if(typeof opt.callback === 'function' && $('[id^=popup]').length === 0) opt.callback();
                });
            ...
            container.show(0).css({opacity: 0, top: '100%'});
            var popupCount = container.find('.swiper-slide').length;
            var duration = 5000, speed = 500;
            swiper = new Swiper(container.find('.swiper-container'), {
                setWrapperSize: true,
                autoplay: {
					delay: 5000
                ...
                on: {
					slideChangeTransitionEnd: function(s) {
	                    if(swiper.activeIndex === popupCount - 1) {
            ...
            swiper.autoplay.stop();
            container.animate({opacity: 1, top: 0}, 500, function() {
                $('.' + type + '_timebar').animate({width: '100%'}, (popupCount * duration) + ((popupCount - 1) * speed), 'linear', function() {
                    $('#popupX' + type).trigger('click');
                });
                swiper.autoplay.stop();
            });
			$('#popupCheck' + type).click(close);
			$('#popupX' + type).click(close);
		};
...
(function($) {
	...
	window.Sitemap = function() {
		 ...
		 */
		this.initialize = function() {
			initIdx = $(_depth1Li).filter('.on').index();
			$topElement.on('click', _anchor, this.spread);
			$topElement.on('click', _closer, this.fold);
            $topElement.on('click', _closer_li, this.fold_li);
			$topElement.on('click', '.stm_depth1_a', function(e) {
				var self = $(this).closest(_depth1Li);
				if(self.hasClass('on') === false && self.find(_depth2Ul).length > 0 && tween === false) {
					$(_depth2Ul).filter(':visible').slideUp(200);
					self.find(_depth2Ul).slideDown(200, function() {
						tween = false;
					});
					$(_depth1Li).filter('.on').removeClass('on');
					self.addClass('on');
					tween = true;
				}
				e.preventDefault();
...
(function($) {
	$(document).ready(function() {
		...
		(function() {
            ...
            $('.login_bg,.login_popup .close').click(function(){
                $('.login_popup').removeClass('on');
                $('html,body').removeClass('no_scroll');
            });
		}());
